...
class Doctor:
    def __init__(self, doctor_dict=None):
        self.test_result = {
    ...
    def set_test_results(self):
        ...
        if 0 <= self.patient_info["user_age"] <= 3:
            if self.patient_info["WBC"] < 6000:
                self.test_result['WBC'] = -1
            elif self.patient_info["WBC"] > 17500 * 1.5:
                self.test_result['WBC'] = 2
            elif self.patient_info["WBC"] > 17500:
                self.test_result['WBC'] = 1
        elif 4 <= self.patient_info["user_age"] <= 17:
            if self.patient_info["WBC"] < 5500:
                self.test_result['WBC'] = -1
            elif self.patient_info["WBC"] > 15500 * 1.5:
                self.test_result['WBC'] = 2
            elif self.patient_info["WBC"] > 15500:
                self.test_result['WBC'] = 1
        elif self.patient_info["user_age"] >= 18:
            if self.patient_info["WBC"] < 4500:
                self.test_result['WBC'] = -1
            elif self.patient_info["WBC"] > 11000 * 1.5:
                self.test_result['WBC'] = 2
            elif self.patient_info["WBC"] > 11000:
                self.test_result['WBC'] = 1

        # Neut rating
        if self.patient_info["Neut"] < 28:
            self.test_result['Neut'] = -1
        elif self.patient_info["Neut"] > 54:
            self.test_result['Neut'] = 1

        # Lymph rating
        if self.patient_info["Lymph"] < 36:
            self.test_result['Lymph'] = -1
        elif self.patient_info["Lymph"] > 52:
            self.test_result['Lymph'] = 1

        # RBC rating
        if self.patient_info["RBC"] < 4.5:
            self.test_result['RBC'] = -1
        elif self.patient_info["RBC"] > 6:
            self.test_result['RBC'] = 1

        # HCT rating
        if self.patient_info["user_gender"] == 'm':
            if self.patient_info["HCT"] < 37:
                self.test_result['HCT'] = -1
            elif self.patient_info["HCT"] > 54:
                self.test_result['HCT'] = 1
        elif self.patient_info["user_gender"] == 'w':
            if self.patient_info["HCT"] < 33:
                self.test_result['HCT'] = -1
            elif self.patient_info["HCT"] > 47:
                self.test_result['HCT'] = 1

        # Urea rating
        if self.patient_info["user_eastern_community"] == 0:
            if self.patient_info["Urea"] < 17:
                self.test_result['Urea'] = -1
            elif self.patient_info["Urea"] > 43:
                self.test_result['Urea'] = 1
        elif self.patient_info["user_eastern_community"] == 1:
            if self.patient_info["Urea"] < 17 * 1.1:
                self.test_result['Urea'] = -1
            elif self.patient_info["Urea"] > 43 * 1.1:
                self.test_result['Urea'] = 1

        # Hb rating
        if self.patient_info["user_gender"] == 'w':
            if self.patient_info["Hb"] < 12:
                self.test_result['Hb'] = -1
            elif self.patient_info["Hb"] > 16:
                self.test_result['Hb'] = 1
        elif self.patient_info["user_gender"] == 'm':
            if self.patient_info["Hb"] < 12:
                self.test_result['Hb'] = -1
            elif self.patient_info["Hb"] > 18:
                self.test_result['Hb'] = 1
        elif 0 <= self.patient_info["user_age"] <= 17:
            if self.patient_info["Hb"] < 11.5:
                self.test_result['Hb'] = -1
            elif self.patient_info["Hb"] > 15.5:
                self.test_result['Hb'] = 1

        # Crtn rating
        if 0 <= self.patient_info["user_age"] <= 2:
            if self.patient_info["Crtn"] < 0.2:
                self.test_result['Crtn'] = -1
            elif self.patient_info["Crtn"] > 0.5:
                self.test_result['Crtn'] = 1
        elif 3 <= self.patient_info["user_age"] <= 17:
            if self.patient_info["Crtn"] < 0.5:
                self.test_result['Crtn'] = -1
            elif self.patient_info["Crtn"] > 1:
                self.test_result['Crtn'] = 1
        elif 18 <= self.patient_info["user_age"] <= 59:
            if self.patient_info["Crtn"] < 0.6:
                self.test_result['Crtn'] = -1
            elif self.patient_info["Crtn"] > 1:
                self.test_result['Crtn'] = 1
            ...
            if self.patient_info["Crtn"] < 0.6:
                self.test_result['Crtn'] = -1
            elif self.patient_info["Crtn"] > 1.2:
                self.test_result['Crtn'] = 1

        # Iron rating
        if self.patient_info["user_gender"] == 'm':
            if self.patient_info["Iron"] < 60:
                self.test_result['Iron'] = -1
            elif self.patient_info["Iron"] > 160:
                self.test_result['Iron'] = 1
        elif self.patient_info["user_gender"] == 'w':
            if self.patient_info["Iron"] < 60 * 0.8:
                self.test_result['Iron'] = -1
            elif self.patient_info["Iron"] > 160 * 0.8:
                self.test_result['Iron'] = 1

        # HDL rating
        if self.patient_info["user_ethiopian_community"] == 0:
            if self.patient_info["user_gender"] == 'm':
                if self.patient_info["HDL"] < 29:
                    self.test_result['HDL'] = -1
                elif self.patient_info["HDL"] > 62:
                    self.test_result['HDL'] = 1
            elif self.patient_info["user_gender"] == 'w':
                if self.patient_info["HDL"] < 34:
                    self.test_result['HDL'] = -1
                elif self.patient_info["HDL"] > 82:
                    self.test_result['HDL'] = 1
        elif self.patient_info["user_ethiopian_community"] == 1:
            if self.patient_info["user_gender"] == 'm':
                if self.patient_info["HDL"] < 29 * 1.2:
                    self.test_result['HDL'] = -1
                elif self.patient_info["HDL"] > 62 * 1.2:
                    self.test_result['HDL'] = 1
            elif self.patient_info["user_gender"] == 'w':
                if self.patient_info["HDL"] < 34 * 1.2:
                    self.test_result['HDL'] = -1
                elif self.patient_info["HDL"] > 82 * 1.2:
                    self.test_result['HDL'] = 1

        # AP rating
        if self.patient_info["user_eastern_community"] == 1:
            if self.patient_info["AP"] < 60:
                self.test_result['AP'] = -1
            elif self.patient_info["AP"] > 120:
                self.test_result['AP'] = 1
        elif self.patient_info["user_eastern_community"] == 0:
            if self.patient_info["AP"] < 30:
                self.test_result['AP'] = -1
            elif self.patient_info["AP"] > 90:
                self.test_result['AP'] = 1
    ...
    def get_input(self, file_name):
        df = pd.read_excel(self.excel_path + r"\{0}".format(file_name))
        temp = df.to_dict()
        ...
        # list(item[1].values())[0])
        for item in temp.items():
            data[item[0]] = list(item[1].values())[0]
        return data
